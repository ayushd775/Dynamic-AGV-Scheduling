...
def execute_movement(agv, path, task, loaded=False):
    ...
    try:
        for i in range(len(path) - 1):
            ...
            # Find available time for edge and node
            ...
            # Log and reserve
            log_movement(agv, start_node, end_node, actual_start, task['ID'], task['Payload Weight'], travel_time)
    ...
    for task in tasks:
        ...
        for agv in agv_list:
            try:
                ...
                battery_needed = (
                    sum(edge[1] * (1 + 0/10) for edge in path_edges(path_to_source)) +
                    sum(edge[1] * (1 + task['Payload Weight']/10) for edge in path_edges(path_to_dest))
                ) * SAFETY_MARGIN

                # 3. Critical check: Can this task ever be done?
                if battery_needed > MAX_BATTERY:
                    print(f"Task {task['ID']} requires {battery_needed}min battery (max {MAX_BATTERY}min). Skipping.")
                    continue

                # 4. Charge until sufficient battery
                while agv.battery < battery_needed:
                    prev_battery = agv.battery
                    charge_agv(agv)
                    if agv.battery <= prev_battery:  # Charging failed to increase
